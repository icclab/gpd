...
#            print("rotation: " + str(rot_quat))
         ...
         #   rot_x_q = Quaternion(0.7071, 0.7071, 0, 0)  # 90* around Z axis (W, X, Y, Z)
          ...
          #  quat = org_q
            ...
            #g.grasp_pose = gp
            # transform grasp pose to our move_it planning frame
            
            listener.waitForTransform('/arm_camera_depth_optical_frame', '/summit_xl_base_footprint', rospy.Time(), rospy.Duration(1.0))
...
#            print("Original grasp: " + str(single_grasp.grasp_pose))
            adapted_grasp = copy.copy(single_grasp)
            grasp_pose = adapted_grasp.grasp_pose.pose
            orig = (grasp_pose.orientation.x, grasp_pose.orientation.y, grasp_pose.orientation.z, grasp_pose.orientation.w) # x,y,z,w
            q_rot = quaternion_from_euler(0, 0, 0) # rotate 90 degrees around y axis
#            print("rotation quaternion +90 degrees around y (x,y,z,w): " + str(q_rot))
            rotated_pose = quaternion_multiply(orig, q_rot) 
#            print(rotated_pose)
            grasp_pose.orientation.w = float(rotated_pose[3])
            grasp_pose.orientation.x = float(rotated_pose[0])
            grasp_pose.orientation.y = float(rotated_pose[1])
            grasp_pose.orientation.z = float(rotated_pose[2])
#            print("Adapted grasp: " + str(adapted_grasp.grasp_pose))
            pick_result = self.p.pickup("obj", [adapted_grasp, ], planning_time=9001, support_name="<octomap>", allow_gripper_support_collision=True)
...
#        print("Initial pose: " + str(pose_goal))
